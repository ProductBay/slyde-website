...
function renderContent(content: string) {
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    if (isBulletBlock || isNumberedBlock) {
          ...
          {lines.map((line, li) => {
            ...
            return (
              <li key={li} className="flex min-w-0 gap-2.5 text-sm leading-6 text-slate-700">
                ...
                <span className="min-w-0 break-words" dangerouslySetInnerHTML={{ __html: formatInline(text) }} />
              </li>
            );
          })}
        </ul>,
      ...
      nodes.push(
        <p key={i} className="mt-3 break-words text-sm leading-7 text-slate-700" dangerouslySetInnerHTML={{ __html: formatInline(para) }} />,
      );
    }
  }
...
type Props = { articles: SupportKnowledgeArticle[] };

function ArticleModal({ article, onClose }: { article: SupportKnowledgeArticle; onClose: () => void }) {
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[9999] flex flex-col items-stretch justify-end bg-slate-950/60 sm:items-center sm:justify-center sm:p-6"
      onClick={onClose}
    >
      <div
        className="flex w-full flex-col overflow-hidden rounded-t-[1.75rem] bg-white shadow-2xl sm:max-w-2xl sm:rounded-[1.75rem]"
      ...
      >
            ...
            {article.tags.length > 0 && (
              <div className="mb-2 flex flex-wrap gap-1.5">
                {article.tags.map((tag) => (
                  <span key={tag} className="rounded-full bg-sky-50 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-sky-700">
                    {tag}
                  </span>
                ))}
              </div>
            )}
            <h2 className="break-words text-base font-semibold leading-snug text-slate-950 sm:text-lg">{article.title}</h2>
            {article.summary && <p className="mt-1 break-words text-sm text-slate-500">{article.summary}</p>}
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        >
          <p className="text-xs text-slate-400">
            Need more help?{" "}
            <a href="/support" className="text-sky-600 underline-offset-2 hover:underline">Submit a request</a>
          </p>
          <button
            type="button"
            onClick={onClose}
